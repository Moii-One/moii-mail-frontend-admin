...
export function requiresAuth(to: any): boolean {
    ...
    const packagePublicRoutes: string[] = [
        // Add any public users routes here if needed
    ];
...
export function getLoginRedirect(to: any): string {
    ...
    const redirectPath = to.path !== '/login' ? to.fullPath : '/users';
    return `/login${redirectPath !== '/users' ? `?redirect=${encodeURIComponent(redirectPath)}` : ''}`;
}

...
export function requiresAuth(to: any): boolean {
    ...
    const packagePublicRoutes: string[] = [
        // Add any public localizations routes here if needed
    ];
    ...
    if (to.path.startsWith('/localizations')) {
        return true;
    }
...
export function getLoginRedirect(to: any): string {
    ...
    const redirectPath = to.path !== '/login' ? to.fullPath : '/localizations';
    return `/login${redirectPath !== '/localizations' ? `?redirect=${encodeURIComponent(redirectPath)}` : ''}`;
}

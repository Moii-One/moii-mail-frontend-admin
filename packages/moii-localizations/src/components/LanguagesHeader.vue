    ...
    <div class="space-y-4">
        <div class="panel flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-white-dark text-sm mt-1">Manage application languages and locale settings</p>
            </div>
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
                <button 
                    v-if="hasPermission('languages.create')"
                    type="button" 
...
const emit = defineEmits<{
    (e: 'update:filters', value: LanguageFilterModel): void;
    (e: 'add'): void;
}>();
...
const typeOptions = [
    ...
    { value: 'default', label: 'Default Language' },
    { value: 'non-default', label: 'Non-Default Languages' }
];

const updateFilter = (key: keyof LanguageFilterModel, value: string) => {
    emit('update:filters', {
        ...props.filters,
        [key]: value
    });

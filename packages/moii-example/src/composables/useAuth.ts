...
export function getLoginRedirect(to: any): string {
    ...
    const redirectPath = to.path !== '/login' ? to.fullPath : '/example/dashboard';
    return `/login${redirectPath !== '/example/dashboard' ? `?redirect=${encodeURIComponent(redirectPath)}` : ''}`;
}

...
export function requiresAuth(to: any): boolean {
    ...
    const packagePublicRoutes: string[] = [
        // Add any public settings routes here if needed
    ];
...
export function getLoginRedirect(to: any): string {
    ...
    const redirectPath = to.path !== '/login' ? to.fullPath : '/settings';
    return `/login${redirectPath !== '/settings' ? `?redirect=${encodeURIComponent(redirectPath)}` : ''}`;
}

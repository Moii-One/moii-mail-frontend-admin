...
export function requiresAuth(to: any): boolean {
    ...
    const packagePublicRoutes: string[] = [
        // Add any public rate limits routes here if needed
    ];
    ...
    if (to.path.startsWith('/rate-limits')) {
        return true;
    }
...
export function getLoginRedirect(to: any): string {
    ...
    const redirectPath = to.path !== '/login' ? to.fullPath : '/rate-limits';
    return `/login${redirectPath !== '/rate-limits' ? `?redirect=${encodeURIComponent(redirectPath)}` : ''}`;
}

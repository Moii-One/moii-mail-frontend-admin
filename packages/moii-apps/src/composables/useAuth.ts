...
export function requiresAuth(to: any): boolean {
    ...
    const packagePublicRoutes: string[] = [
        // Add any public apps routes here if needed
    ];
...
export function getLoginRedirect(to: any): string {
    ...
    const redirectPath = to.path !== '/login' ? to.fullPath : '/apps';
    return `/login${redirectPath !== '/apps' ? `?redirect=${encodeURIComponent(redirectPath)}` : ''}`;
}

                ...
                <div class="relative flex flex-col justify-center rounded-md bg-white/60 backdrop-blur-lg dark:bg-black/50 px-6 lg:min-h-[758px] py-20">
                    ...
                    <div class="mx-auto w-full max-w-[440px]">
                        ...
                        <form class="space-y-5 dark:text-white" @submit.prevent="handleLogin">
                            ...
                            <button type="submit" :disabled="authStore.isLoading" class="btn btn-gradient !mt-6 w-full border-0 uppercase shadow-[0_10px_20px_-10px_rgba(67,97,238,0.44)]">
    ...
    const handleLogin = async () => {
        ...
        if (result.success) {
            if (result.requires_2fa) {
                // Redirect to 2FA page with verification UUID
                router.push({
                    name: '2fa',
                    query: { 
                        verification_uuid: result.verification_uuid,
                        expires_at: result.expires_at,
                        redirect: router.currentRoute.value.query.redirect as string
                    }
                });
            } else {
                // Normal login - redirect to intended page or dashboard
